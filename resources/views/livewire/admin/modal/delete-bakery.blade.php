    ...
    <div x-data="{ open: false }">
        ...
        <div x-show="open" x-cloak wire:ignore.self class="fixed inset-0 bg-gray-500 bg-opacity-50 z-40 flex items-center justify-center">
            <div class="bg-white p-5 rounded shadow-md w-1/3 ">
                <p>Are you sure you want to delete this Bakery?</p>
                <div class="mt-4 flex justify-center gap-2">
                    <button @click="open = false" class="bg-gray-300 px-4 py-2 rounded">Cancel</button>
                    <button wire:click="deleteBakery"  @click="open = false; setTimeout(() => { window.location.href = '{{ route('admin.bakery') }}'; }, 500);" class="bg-[#51331b] text-white px-4 py-2 rounded">Delete</button>
                </div>
            </div>
        </div>
